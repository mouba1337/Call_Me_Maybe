...
class ConstrainedEngine(BaseModel):
    ...
    def _mask_logits(
        ...
        return [float(x) for x in np_logits]

    def _determine_expected_type(
        self, current_text: str, schema: List[Dict[str, Any]]
    ) -> str:
        """
        ...
        if last_quote != -1:
            ...
            if current_text.count('"') % 2 == 1:
                ...
                # If quote follows { or ,
                # we're typing an object key, not a value
                if i >= 0 and current_text[i] in "{,":
                    return "string"
                # allow alphabet; do NOT schema-mask as number/bool/etc.
    ...
    def _get_cached_allowed_tokens(
        self,
        key: str,
        ...
        if state == ParserState.EXPECTING_PARAM_VALUE:
            ...
            if expected_type in ("number", "integer", "float"):
                return self._get_cached_allowed_tokens(
                    "value:number",
                    lambda: self.vocab_filter.get_tokens_by_chars(
                        "0123456789.- \t\n,}"
                    ),
                )
    ...
    def generate_function_call(
        self, prompt: str, schema: List[Dict[str, Any]], max_tokens: int = 120
    ) -> Optional[str]:
        try:
            self.token_set_cache.clear()
            sys_prompt = (
                "You are a strict JSON formatting AI. "
                "Output ONLY a valid JSON object.\n"
                'CRITICAL: Preserve exact wording from the user request, '
                'including escaping literal quotes (\\").\n'
            ...
            for _ in range(max_tokens):
                ...
                # 5. Token masking
                allowed_ids = self._determine_allowed_tokens(
                    current_state,
                    current_text, schema,
